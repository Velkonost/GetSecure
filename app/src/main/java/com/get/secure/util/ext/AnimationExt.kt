package com.get.secure.util.ext
...
fun View.setTouchAnimation() {
    setOnTouchListener { v, event ->
        when(event.action) {
            MotionEvent.ACTION_DOWN -> animate().scaleY(0.9f).scaleX(0.9f).duration = 100
            else -> animate().scaleY(1f).scaleX(1f).duration = 100
        }
        false
    }
}